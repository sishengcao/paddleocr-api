...
## 13. 2026-01-07 容器启动失败问题（Docker Compose 部署）

### 问题描述

通过 Docker Compose 部署后，多个容器不停重启，服务无法使用。

```
CONTAINER ID   IMAGE               STATUS
paddleocr-api      Restarting (1)
paddleocr-celery-beat   Restarting (2)
paddleocr-celery-worker   Up 3 seconds (health: starting)
paddleocr-mysql    Up 5 minutes (healthy)
paddleocr-redis    Up 5 minutes (healthy)
```
...
**原因**：Dockerfile 中使用了 `libgl1-mesa-glx`，在 Debian trixie 中此包已弃用。

#### 2. 硬编码的日志路径

**错误信息**：
```
FileNotFoundError: [Errno 2] No such file or directory: '/mnt/d/project/github/paddleocr-api/logs/celery_worker.log'
```

**原因**：`app/workers/celery_worker.py` 中日志路径被硬编码为开发机器路径。

### 解决方案

#### 修复 Dockerfile

将 `libgl1-mesa-glx` 改为 `libgl1`：
...
```
CONTAINER ID   IMAGE               STATUS
paddleocr-api      Up 30 seconds (healthy)
paddleocr-celery-worker   Up 32 seconds (health: starting)
paddleocr-celery-beat     Up 32 seconds (health: starting)
paddleocr-mysql    Up 5 minutes (healthy)
paddleocr-redis    Up 5 minutes (healthy)
```
...
| 问题 | 原因 | 解决方案 |
|------|------|----------|
| libGL.so.1 缺失 | Debian trixie 中 libgl1-mesa-glx 已弃用 | 使用 libgl1 替代 |
| 日志路径错误 | 硬编码开发机器路径 | 使用容器内路径 /app/logs |

### 预防措施

1. **避免硬编码路径**：使用环境变量或相对路径
2. **测试不同基础镜像**：确保在目标 Debian 版本上测试
3. **使用健康检查**：快速发现容器启动问题
4. **查看容器日志**：`docker logs <container>` 是排查问题的关键

### 相关文件

- `Dockerfile` - 已修复 libgl1 依赖
- `app/workers/celery_worker.py` - 已修复日志路径
- `docker-compose.yml` - 定义服务依赖和健康检查
